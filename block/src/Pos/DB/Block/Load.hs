...
getBlundThrow hash = do
    ...
    whenNothing_ mBlund $ reportOrLogE formattedErr $ throw (DBMalformed formattedErr)
    maybeThrow (DBMalformed formattedErr)  mBlund
  where
    formattedErr = sformat errFmt hash
